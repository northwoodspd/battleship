...
describe SeanBotPlayer do
  ...
  describe '#add_ship' do
    before do
      allow_any_instance_of(Array).to receive(:sample).and_return random_orientation
    end

    context 'when ships are going down' do
      let(:random_orientation) { :down }

      it 'returns the ship structure' do
        allow(subject).to receive(:rand).and_return 2

        expect(subject.add_ship(2)).to eq [2, 0, 2, :down]
      end
      ...
      it 'uses a random top-left starting point' do
        ...
        expect(subject.add_ship(2)).to eq [7, 0, 2, :down]
      end
      ...
      it 'keeps selecting locations until one will not go off the bottom of the board' do
        ...
        expect(subject.add_ship(2)).to eq [0, 0, 2, :down]
      end
    end

    context 'when ships are going across' do
      let(:random_orientation) { :across }
      ...
      it 'keeps selecting locations until one will not go off the edge of the board' do
        ...
        expect(subject.add_ship(2)).to eq [0, 0, 2, :across]
      end
      ...
      it 'keeps selecting locations until it does not overlap with other ships' do
        ...
        subject.add_ship(5) # 5 long at [1, 0]
        final_coords = subject.add_ship(4) # 4 long at [0, 0], [0, 0], then [1, 1]
